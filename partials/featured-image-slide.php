...
<figure class="c-featured <?php ( get_query_var( 'class' ) == true ) ? print get_query_var( 'class') : ""; ?> <?php ( get_query_var( 'margin' ) == true ) ? print "u-margin" : ""; ?> ">
  <picture>
    <?php if ( $img_slide ) { ?>
    <source media="(min-width: 60rem)" srcset="<?php echo $img_slide[0]; ?>">
    <?php }
    if ( $img_slide_medium ) { ?>
    <source media="(min-width: 40rem)" srcset="<?php echo $img_slide_medium[0]; ?>">
    <?php }
    if ( $img_slide_small ) { ?>
    <source srcset="<?php echo $img_slide_small[0]; ?>">
    <?php }
    if ( $img_slide ) { ?>
    <img srcset="<?php echo $img_slide[0]; ?>" alt="<?php the_title(); ?>" width="<?php echo $img_slide[1]; ?>" height="<?php echo $img_slide[2]; ?>">
    <?php } ?>
  </picture>
</figure>
<?php } ?>