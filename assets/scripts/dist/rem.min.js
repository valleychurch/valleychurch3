!function(window, undefined) {
    ...
    var cssremunit = function() {
        ...
        for (var i = 0; i < links.length; i++) xhr(links[i], storeCSS);
        ...
        for (var i = 0; i < foundProps.length; i++) css[i] = Math.round(parseFloat(foundProps[i].substr(0, foundProps[i].length - 3) * fontSize)) + "px";
        loadCSS();
    }, loadCSS = function() {
        ...
        try {
            ...
            xhr.open("GET", url, !0), xhr.onreadystatechange = function() {
                4 === xhr.readyState && callback(xhr, url);
            }, xhr.send(null);
        } catch (e) {
        ...
        return window.matchMedia || window.msMatchMedia || (css = css.replace(/@media[\s\S]*?\}\s*\}/g, "")), 
        css;
    };
    if (!cssremunit()) {
        var rules = "", links = isStyleSheet(), importLinks = [], found = [], foundProps = [], preCSS = [], CSSLinks = [], css = [], fontSize = "";
        fontSize = function() {
            var size, doc = document, docElement = doc.documentElement, body = doc.body || doc.createElement("body"), isFakeBody = !doc.body, div = doc.createElement("div"), currentSize = body.style.fontSize;
            return isFakeBody && docElement.appendChild(body), div.style.cssText = "width:1em; position:absolute; visibility:hidden; padding: 0;", 
            body.style.fontSize = "1em", body.appendChild(div), size = div.offsetWidth, isFakeBody ? docElement.removeChild(body) : (body.removeChild(div), 
            body.style.fontSize = currentSize), size;
        }(), processLinks();
    }
}(window);