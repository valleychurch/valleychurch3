...
define( 'VC_THEME_VERSION', '3.6.2' );
define( 'CS_APP_ACCOUNT', 'valley' );
define( 'CS_APP_APPLICATION', 'valleychurch-website' );
define( 'CS_APP_AUTH', 'Dg8lHr5mIg30qcVdN7Je' );

define( 'CS_EVENTS_URL', 'https://api.churchsuite.co.uk/v1/calendar/events?per_page=200&group_by_sequence=true&categories=8,9,12,14,15' );
define( 'CS_GROUPS_URL', 'https://api.churchsuite.co.uk/v1/smallgroups/groups?view=active_future' );
...
function create_custom_post_types() {
  ...
  register_post_type( 'bestsummerever', create_custom_post_type_args( 'bestsummerever', 'Best Summer Ever Event', 'dashicons-palmtree', true, null, null, true ) );
};
...
function my_rest_prepare_podcast($data, $post, $request) {
  ...
  if (!empty($powerpress_data['duration_mm'] && !empty($powerpress_data['duration_ss']))) {
    $_data['powerpress_duration'] = $powerpress_data['duration_mm'] . ':' . $powerpress_data['duration_ss'];
  }
      ...
      if ( $wp_group->post_count == 0 ) {
        $post_id = wp_insert_post(
          array(
            ...
            'post_status' => 'publish',
            'post_author' => 2,
            'post_date' => $group->signup_date_start,
            'comment_status' => 'closed',
            'ping_status' => 'closed',
          )
        );
        ...
        if ( $post_id ) {
          ...
          if ( $group->day !== null ) {
            ...
            update_field( 'field_59b642f0ed991', 7, $post_id );
          }
          ...
          if ($group->signup_full) {
            ...
            update_field( 'field_59b64354ed993', 0, $post_id );
          }
...
add_action( "import_churchsuite_groups", "import_churchsuite_groups" );
add_action( "wp_ajax_import_churchsuite_groups", "import_churchsuite_groups" );
add_action( "wp_ajax_nopriv_import_churchsuite_groups", "import_churchsuite_groups" );
